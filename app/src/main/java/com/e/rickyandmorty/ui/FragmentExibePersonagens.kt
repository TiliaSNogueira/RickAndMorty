...
class FragmentExibePersonagens : Fragment(), AdapterTodosPersonagens.personagemOnClickListener {
    ...
    override fun escolhePersonagemClick(position: Int) {
        viewModel.todosPersonagens.observe(this, {
            val personagemEscolhido = it.get(position)

            //colocando o restaurante escolhido no bundle
            val bundlePersonageEscolhido = bundleOf("chave" to personagemEscolhido.id)

            findNavController().navigate(R.id.action_fragmentExibePersonagens_to_fragmentDetalhesPersonagem, bundlePersonageEscolhido)
